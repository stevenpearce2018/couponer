const validateCouponForm = state => {
  ...
  else if (Number(state.discountedPrice) === Number(state.currentPrice) ) return false;
  else if (Number(state.discountedPrice) <= Number(state.currentPrice) ) return false;
  else if (Number(state.discountedPrice) === 0 || Number(state.currentPrice) === 0) return false;
